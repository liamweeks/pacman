...
#define DOCTEST_INTERNAL_ERROR(msg)                                                                \
    ...
    using MultiLaneAtomic = Atomic<T>;
        ...
        public:
            ...
            DOCTEST_NOEXCEPT { return fetch_add(1) + 1; }

            T operator++(int)

            DOCTEST_NOEXCEPT { return fetch_add(1); }
    ...
    noexcept {
    *reinterpret_cast
    <unsigned char *>(&buf[last]) = 128;
}

void String::setLast(size_type in)
...
struct ConsoleReporter : public IReporter {
    ...
    void test_run_end(const TestRunStats &p) override {
        ...
        auto totwidth = int(std::ceil(
                log10(static_cast<double>(std::max(p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts))) +
                      1)));
        ...
        auto failwidth = int(std::ceil(
                log10(static_cast<double>(std::max(p.numTestCasesFailed, static_cast<unsigned>(p.numAssertsFailed))) +
                      1)));
